		...
		for _,doorID in ipairs(Config.DoorList) do
			if doorID.doors then
				for k,v in ipairs(doorID.doors) do
					if not v.object or not DoesEntityExist(v.object) or not v.objHash then
						v.object = GetClosestObjectOfType(v.objCoords, 1.0, GetHashKey(v.objName), false, false, false)
					end
					if not v.object or not DoesEntityExist(v.object) and v.objHash then
						v.object = GetClosestObjectOfType(v.objCoords, 1.0, v.objHash, false, false, false)
					end
				end
			else
				if not doorID.object or not DoesEntityExist(doorID.object) or not doorID.objHash then
					doorID.object = GetClosestObjectOfType(doorID.objCoords, 1.0, GetHashKey(doorID.objName), false, false, false)
				end
				if not doorID.object or not DoesEntityExist(doorID.object) and doorID.objHash then
...
Citizen.CreateThread(function()
	while true do
		...
		for k,doorID in ipairs(Config.DoorList) do
			...
			local maxDistance, size, displayText = 1.25, 0.7, _U('unlocked')
			...
			if distance < maxDistance then
				...
				if doorID.textCoords2 then 
					ESX.Game.Utils.DrawText3D(doorID.textCoords, displayText, size)
					ESX.Game.Utils.DrawText3D(doorID.textCoords2, displayText, size)
				else
					ESX.Game.Utils.DrawText3D(doorID.textCoords, displayText, size)
				end
